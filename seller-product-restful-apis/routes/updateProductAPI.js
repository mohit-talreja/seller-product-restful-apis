...
router.put('/:productId', verifyToken, upload.single('productpic'), async (req, res) => {
    try {
        ...
        if(authData){
            ...
            const updateObj = await Product.updateOne({ _id: productId }, {
                $set: { title, price, description, productpic: filename} 
            })
            res.status(200).json({
                message: `${authData.email} Is A Verified Seller, ${updateObj.nModified} Product Updated`
            })
        } else {
            res.sendStatus(403)
        }
